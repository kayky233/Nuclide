...
:G4UImessenger(), 
 ...
 fDetectRadiusCmd(0), fDetectMatCmd(0), fDetectLengthCmd(0) 
{ 
  ...
  fDetectRadiusCmd =
       new G4UIcmdWithADoubleAndUnit("/rdecay02/det/setDetectorRadius",this);
  fDetectRadiusCmd->SetGuidance("Set the Detector Radius.");
  fDetectRadiusCmd->SetUnitCategory("Length");
  fDetectRadiusCmd->SetParameterName("choice",false);
  fDetectRadiusCmd->AvailableForStates(G4State_PreInit);
...
{ 
  ...
  if (command == fDetectRadiusCmd ) 
    {fDetector->SetDetectorRadius(
                     fDetectRadiusCmd->GetNewDoubleValue(newValue));}      
}
