...
# sys的作用：可以在命令行输入训练文件夹和答案： python3 01_make_slice_data.py train_folder train_answr 以覆盖上面默认的训练集文件夹
if len(sys.argv) > 1:
...
# training folder 命名： RunID.csv
    ...
    # divide test file into 30 segments 把训练集分为30份
    num_seg = 30
    ...
    if source_id == 0:
        ...
        # find 30 sec index
        #  there will be no source located within the first 30 seconds of measurements.
        # 因为前30s没有数据，为背景噪声，所以选到30s这个节点
        df_sort = df.iloc[(df['time_cumsum'] - 30000000).abs().argsort()[:2]]
        #选择df_sort.index.tolist()[0]即27678为30s的节点
        ...
        df_sort = df.iloc[(df['time_cumsum'] - source_time).abs().argsort()[:2]]
